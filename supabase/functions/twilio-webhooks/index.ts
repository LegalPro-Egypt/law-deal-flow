...
async function handleParticipantDisconnected(supabaseClient: any, participantSid: string) {
  ...
  // Find participant by identity (we'll need to store participant_sid when they connect)
  const { data: participant } = await supabaseClient
    .from('twilio_session_participants')
    .select('joined_at')
    .eq('participant_identity', participantSid) // This might need adjustment based on how we store participant data
    .single();
