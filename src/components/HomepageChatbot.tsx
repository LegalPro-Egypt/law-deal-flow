...
const HomepageChatbot: React.FC<HomepageChatbotProps> = ({ 
  className, 
  spinOnScroll3D = true,
  isModal = false,
  open = false,
  ...
  const chatInterface = (
    <Card 
      id="lexa-chat"
      className={`shadow-lg border-0 overflow-hidden bg-background ${!isModal ? 'transition-all duration-1000 delay-500' : ''} ${
        !isModal && isVisible ? 'animate-slide-up-fade opacity-100' : !isModal ? 'opacity-0 translate-y-16' : ''
      }`}
    >
      {/* Professional Header */}
  ...
  // Return modal version if isModal prop is true
  if (isModal) {
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-4xl max-h-[90vh] p-0 overflow-hidden">
          <DialogHeader className="sr-only">
            <DialogTitle>{t('landing.chatbot.lexaTitle')}</DialogTitle>
          </DialogHeader>
          {chatInterface}
        </DialogContent>
      </Dialog>
  ...
  // Always visible professional chat interface (standalone section)
  return (
    <section ref={sectionRef as React.RefObject<HTMLElement>} className={`py-16 bg-muted/30 ${className} ${isRTL() ? 'rtl' : 'ltr'}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto">
          ...
          {/* Chat Interface Card */}
          <ChatFlip3D flips={1} spinOnScroll3D={spinOnScroll3D} trigger={isVisible}>
            {chatInterface}
          </ChatFlip3D>
        </div>
      </div>
